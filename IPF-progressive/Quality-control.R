setwd("~/GitHub/Imperial-Molyneaux/IPF-progressive//")
setwd("~/Documents/GitHub/Imperial-Molyneaux/IPF-progressive/") # Personal device
...
ggplot(data=df, aes(x=Index, y=LibrarySize, colour = Diagnosis)) + geom_point() +  ylim(lower = 0, upper = 200000) #somewhat rarecurve?

sample_data(ps)$is.neg <- sample_data(ps)$`Diagnosis`=="Negative" #Only considered reagent control as negative
...
ps.pa.neg <- prune_samples(sample_data(ps.pa)$`Diagnosis`=="Negative", ps.pa)
ps.pa.pos <- prune_samples(!sample_data(ps.pa)$`Diagnosis`=="Negative", ps.pa)
