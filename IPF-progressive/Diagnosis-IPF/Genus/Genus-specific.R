setwd("~/GitHub/Imperial-Molyneaux/IPF-progressive/Diagnosis-IPF/Genus/")
...
ggplot(df, aes(x = Level1, y = Streptococcus)) + 
  ...
  scale_colour_manual(values=c("darkorange","steelblue", "orchid")) + 
  scale_fill_manual(values=c("white", "white","white", "white")) +
  theme_classic() + 
  labs(title="Streptococcus relative abundance by Diagnosis==IPF and Sequencing run",
       y = "Streptococcus relative abundance (%)",
       x = "Level 1 Diagnosis") +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        legend.position = "none")

ggsave("Streptococcus- IPF vs HC by MSQrun.pdf",
       width=300,
       height=200,
       units = c("mm"), dpi=400)
...
wilcox.test(IPF$Streptococcus, HC$Streptococcus) #0.337
...
ggplot(df, aes(x = Level1, y = Veillonella)) + 
  ...
  scale_fill_manual(values=c("white", "white","white", "white")) +
  theme_classic() + 
  labs(title="Veillonella relative abundance by Diagnosis==IPF and Sequencing run",
       y = "Veillonella relative abundance (%)",
       x = "Level 1 Diagnosis") +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        legend.position = "none")

ggsave("Veillonella- IPF vs HC by MSQrun.pdf",
       width=300,
       height=200,
       units = c("mm"), dpi=400)
...
wilcox.test(IPF$Veillonella, HC$Veillonella) #0.44
...
ggplot(df, aes(x = Level1, y = Staphylococcus)) + 
  ...
  scale_fill_manual(values=c("white", "white","white", "white")) +
  theme_classic() + 
  labs(title="Staphylococcus relative abundance by Diagnosis==IPF and Sequencing run",
       y = "Staphylococcus relative abundance (%)",
       x = "Level 1 Diagnosis") +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        legend.position = "none")

ggsave("Staphylococcus- IPF vs HC by MSQrun.pdf",
       width=300,
       height=200,
       units = c("mm"), dpi=400)
...
wilcox.test(IPF$Staphylococcus, HC$Staphylococcus) #0.299
