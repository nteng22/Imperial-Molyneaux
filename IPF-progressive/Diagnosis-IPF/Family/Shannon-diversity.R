#### Housekeeping ####
setwd("~/GitHub/Imperial-Molyneaux/IPF-progressive/Family/")
...
normalised_data <- read_csv("Family-normalised.csv")
...
species_metadata_filter <- species_metadata %>%
  filter(!Level1 == "Negative")
wilcox.test(species_number ~ Level1, data = species_metadata_filter) #p=0.2

ggplot(species_metadata_filter, aes(x=Level1, y=species_number, color=Level1)) +
...
ggsave("SpeciesRichness-Family-unfiltered.pdf", width = 150, height = 150,
...
wilcox.test(shannon_diversity ~ Level1, data = shannon_metadata_filter) # p=0.5

shannon_metadata <- cbind(metadata, shannon_diversity)
shannon_metadata <- shannon_metadata %>%
  filter(!Level1=="Negative") # remove control samples
...
ggsave("ShannonDiversity-Family-unfiltered.pdf", width = 150, height = 150,
       units = c("mm"), device = "pdf")
