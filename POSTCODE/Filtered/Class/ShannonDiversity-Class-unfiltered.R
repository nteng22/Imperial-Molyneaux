...
normalised_data <- read.csv("Class-normalised-unfiltered-metadata.csv")
...
species_metadata_filter <- species_metadata %>%
  filter(Status == "disease" |
           Status == "healthy")
wilcox.test(species_number ~ Status, data = species_metadata_filter) #p=0.04
wilcox.test(species_number ~ Sample.type, data = species_metadata_filter) #p=0.86
...
ggplot(species_metadata, aes(x=Sample.type, y=species_number, color=Sample.type)) +
  ...
  labs(title="Species richness by Class with unfiltered 16S data",
       subtitle = "Wilcoxon rank sum: .~Status p=0.04",
       y = "Species richness",
       x = "Sample type") +
...
ggsave("SpeciesRichness-Class-unfiltered.pdf", width = 150, height = 150,
...
wilcox.test(shannon_diversity ~ Status, data = shannon_metadata_filter) # p=0.15
wilcox.test(shannon_diversity ~ Sample.type, data = shannon_metadata_filter) # p=0.01
...
ggplot(shannon_metadata, aes(x=Sample.type, y=shannon_diversity, color=Sample.type)) +
  ...
  facet_grid(. ~ `Status`,
             scales = "free", space = "free") +
  labs(title="Shannon Diversity by Class with unfiltered 16S data",
       subtitle = "Wilcoxon rank sum: SD~Sample-type p = 0.01",
...
ggsave("ShannonDiversity-Class-unfiltered.pdf", width = 150, height = 150,
       units = c("mm"), device = "pdf")
