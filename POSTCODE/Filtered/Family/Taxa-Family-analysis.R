...
data <- read.csv("Family-normalised-unfiltered-metadata.csv") %>%
...
p <- ggplot(data = dataframe,
  ...
  labs(title = "BAL vs. oral rinse (Unfiltered 16S data)",
       subtitle = "At Family level, no environmental variables were significant") +
...
pdf("16S Unfiltered-Family analysis.pdf", width = 6, height = 6)
