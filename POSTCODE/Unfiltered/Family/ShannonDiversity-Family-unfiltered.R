...
species_metadata_filter <- species_metadata %>%
  filter(Status == "disease" |
           Status == "healthy")
wilcox.test(species_number ~ Status, data = species_metadata_filter) #p=0.4
wilcox.test(species_number ~ Sample.type, data = species_metadata_filter) #p=0.02
...
ggplot(species_metadata, aes(x=Sample.type, y=species_number, color=Sample.type)) +
  ...
  labs(title="Species richness by Family with unfiltered 16S data",
       subtitle = "Wilcoxon rank sum: .~Sample-type p=0.02",
       y = "Species richness",
       x = "Sample type") +
...
wilcox.test(shannon_diversity ~ Sample.type, data = shannon_metadata_filter) # p=0.61
