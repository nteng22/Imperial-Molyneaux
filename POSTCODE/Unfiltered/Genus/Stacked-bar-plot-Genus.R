...
##Select data
normalised_data <- read_csv("Genus-normalised-unfiltered-metadata.csv") %>%
  filter(!Diagnosis == "Negative Control" & !Description == "Mock")
# Prevotella appears twice, Prevotella and [Prevotella]. 
normalised_data$Prevotella <- normalised_data$Prevotella + normalised_data$`[Prevotella]` 
normalised_data <- normalised_data %>%
  select(-"[Prevotella]")

abund_table <- normalised_data %>%
  select(4:ncol(normalised_data))
...
sample_data <- sample_data %>%
  arrange(Prevotella, desc(Veillonella) # Using desc() If you want to arrange in descending order.
  )
...
Palette <- c(Prevotella = "darkseagreen1",
             Oribacterium = "darkslategray2",
             Streptococcus = "hotpink3",
             Veillonella = "goldenrod1",
             Catonella = "palegreen3",
             Unknown = "plum",
             Corynebacterium = "tan2",
             Haemophilus = "darkseagreen4",
             Salinibacterium = "darkslategray4",
             Actinomyces = "deeppink3",
             Others = "grey")
...
ggplot(sample_data_long,
  ...
  labs(title="Ten most abundant Genus",
...
ggsave("Ten most abundant Genus unfiltered.pdf", width = 300, height = 150, units = c("mm"), dpi = 300)
