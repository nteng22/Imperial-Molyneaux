...
data <- read.csv("Genus-normalised-unfiltered-metadata.csv") %>%
  filter(!Diagnosis == "Negative Control") %>%
  filter(!Diagnosis == "Mock") #remove controls

abund_table <- data %>%
  select(4:ncol(data)) # Only select the reads
abund_table$Prevotella <- abund_table$Prevotella + abund_table$X.Prevotella.
...
p <- ggplot(data = dataframe,
  ...
  labs(title = "BAL vs. oral rinse (Unfiltered 16S data)",
       subtitle = "At Genus level, no environmental variables were significant") +
...
pdf("16S Unfiltered-Genus analysis.pdf", width = 6, height = 6)
