setwd("~/GitHub/Imperial-Molyneaux/POSTCODE/Unfiltered/Phylum")
...
data <- read.csv("Phylum-normalised-unfiltered-metadata.csv") %>%
  filter(!Diagnosis == "Negative Control") %>%
  filter(!Diagnosis == "Mock") #remove controls

abund_table <- data %>%
  select(4:ncol(data)) # Only select the reads
abund_table <- dplyr::rename("Thermi" =  "X.Thermi.", abund_table)
...
meta_table$Description <- as.factor(meta_table$Description)
#===============================================================================================================
...
# Subset the dataframe to only those with p<0.05
en_coord_cont_sig <- subset(B_sig_arrows, B$pvals < 0.05) # this will be patient ID
...
dataframe$Diagnosis <- factor(dataframe$Diagnosis,
                                levels = c("CHP",
                                           "Controls",
                                           "COVID", 
                                           "IPF"))
...
p <- ggplot(data = dataframe,
  ...
  # Set the plot title.
  labs(title = "Taxa driven analysis by Diagnosis at phylum level",
       subtitle = "Unfiltered 16S rRNA data") +
...
pdf("16S Unfiltered-Phylum analysis.pdf", width = 6, height = 6)
