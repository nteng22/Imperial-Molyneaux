...
normalised_data <- read.csv("Phylum-normalised-unfiltered-metadata.csv") %>%
  filter(!Diagnosis == "Negative Control" & !Description == "Mock") #remove controls
...
ggplot(data = plot.data, aes(x = NMDS1, y = NMDS2)) + 
  ...
  scale_colour_manual(values = c("deepskyblue3", "hotpink3", "darkgreen", "gold2")) + 
  scale_shape_manual(values = c(1, 1, 1, 1)) +
  labs(title = "NMDS by Phylum of unfiltered taxa",
       subtitle = "PERMANOVA by Diagnosis (p=0.1)") +
