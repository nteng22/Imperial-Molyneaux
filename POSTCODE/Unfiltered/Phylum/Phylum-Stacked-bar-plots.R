...
##Select data
normalised_data <- read_csv("Phylum-normalised-unfiltered-metadata.csv") %>%
  filter(!Diagnosis == "Negative Control" & !Description == "Mock")
normalised_data <- dplyr::rename("Thermi" = "X.Thermi.", normalised_data)
...
ggsave("Ten most abundant Phylum unfiltered.pdf", width = 300, height = 150, units = c("mm"), dpi = 300)
