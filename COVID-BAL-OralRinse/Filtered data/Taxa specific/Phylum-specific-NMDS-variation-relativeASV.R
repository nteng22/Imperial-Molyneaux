...
taxa = "Phylum"
data <- read.csv("../../Phylum-normalised-metadata.csv") %>%
  filter(!Status == "control") # remove controls


meta_table <- data %>%
  select(index:Status)
# Based off of taxa analysis (bionenv), SR1, Bacteroidetes and Firmicutes
  # were the drivers of variance, so extract this from the dataset
abund_table <- data %>%
  select("SR1", "Bacteroidetes", "Firmicutes")
...
plot <- ggplot(data_subset_long, aes(x = `Sample.type`, y = value)) +
...
pdf(file = "Relative ASVs Phylum driving NMDS variation.pdf", width = 11, height = 8)
