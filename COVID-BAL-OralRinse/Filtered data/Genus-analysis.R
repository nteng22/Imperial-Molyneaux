...
data <- read.csv("Genus-normalised-metadata.csv") %>%
...
p <- ggplot(data = dataframe,
  ...
  labs(title = "16S microbiota data from Oral rinse vs. BAL",
       subtitle = "At Genus level, relative proportions of ASVs") +
...
pdf("Filtered data/16S data, oral vs. BAL at Genus, relative ASVs.pdf", width = 6, height = 6)
