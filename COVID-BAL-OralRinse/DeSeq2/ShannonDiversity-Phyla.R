...
### Relative abundances ###
normalised_data <- read.csv("Phylum-DeSeq2normalised-metadata.csv") %>%
  filter(!Status == "control") #remove controls

abundance_table <- normalised_data %>%
  select(5:ncol(normalised_data)) # Only select the reads
rownames(abundance_table) <- normalised_data$index
rowSums(abundance_table)
abundance_table <- abundance_table/colSums(abundance_table)
...
ggsave("ShannonDiversity-relabundances-Phyla.pdf", width = 150, height = 150,
       units = c("mm"), device = "pdf")


### Reads variance stabilised ###
normalised_data <- read.csv("Phylum-DeSeq2normalised-metadata.csv") %>%
  filter(!Status == "control") #remove controls
...
ggsave("ShannonDiversity-reads-Phyla.pdf", width = 150, height = 150,
       units = c("mm"), device = "pdf")
