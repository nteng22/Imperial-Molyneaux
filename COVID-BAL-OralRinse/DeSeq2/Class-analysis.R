...
data <- read.csv("Class-DeSeq2normalised-metadata.csv") %>%
  filter(!Status == "control") #remove controls
data <- read.csv("Class-DeSeq2normalised-relabundance-metadata.csv")
abund_table <- data %>%
...
p <- ggplot(data = dataframe,
  ...
  labs(title = "16S microbiota data from Oral rinse vs. BAL",
       subtitle = "At Class level, DeSeq2 normalised") +
...
pdf("Normalised-DeSeq2 16S data, oral vs. BAL at Phylum.pdf", width = 6, height = 6)
...
# This will print into the current working directory file.
# Set the saved pdf file name and define the size of the plot (height and width are in inches).
pdf("Normalised-DeSeq2 16S data, oral vs. BAL at Class.pdf", width = 6, height = 6)
